...
define('ATTRIBUTE_REGEX', "/^(" . implode('|', $attributes) . ")$/");
define('ATTRIBUTES_REGEX', "/^(" . implode('|', $attributes) . ")e?s$/");
...
*/
    ...
    if(preg_match('/(clean|trim)(.*)/', $key, $m)){
...
class AHTMLNodeList implements Iterator, Countable, ArrayAccess{
  ...
  */
    ...
    if(preg_match(ATTRIBUTES_REGEX, $key, $m) || preg_match('/^((clean|trim).*)s$/', $key, $m)){
...
class CSS{
  ...
  private static function not($str){
    switch(true){
      ...
      default: return self::translate($value);
    }
  }
...
file_get_html('http://www.vinoseleccion.com/regiones/ribera-del-duero');
//file_get_html('http://www.milanuncios.com/pajaros-en-barcelona/agapornies.htm?demanda=n&vendedor=part');

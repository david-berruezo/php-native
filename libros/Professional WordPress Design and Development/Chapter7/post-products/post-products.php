...
function pp_menu() {
	add_options_page(__('Post Products Settings Page','pp-plugin'), __('Post Products Settings','pp-plugin'), 'administrator', __FILE__, 'pp_settings_page');
}

//create post meta box
function pp_init() {
	// create our custom meta box
	add_meta_box('pp-meta',__('Post Product Information','pp-plugin'), 'pp_meta_box','post','side','default');
}
...
function pp_shortcode($atts, $content = null) {
	...
	If ($show == 'sku') {
		...
		$pp_show = $pp_options['currency_sign'].get_post_meta($post->ID,'pp_price',true);
	}elseif ($show == 'weight') {
...
function pp_meta_box($post,$box) {
	...
	echo '<table>';
	echo '<tr>';
	echo '<td>' .__('Sku', 'pp-plugin'). ':</td><td><input type="text" name="pp_sku" value="'.esc_attr($pp_sku).'" size="10"></td>';
	echo '</tr><tr>';
	echo '<td>' .__('Price', 'pp-plugin'). ':</td><td><input type="text" name="pp_price" value="'.esc_attr($pp_price).'" size="5"></td>';
	echo '</tr><tr>';
	echo '<td>' .__('Weight', 'pp-plugin'). ':</td><td><input type="text" name="pp_weight" value="'.esc_attr($pp_weight).'" size="5"></td>';
	echo '</tr><tr>';
	echo '<td>' .__('Color', 'pp-plugin'). ':</td><td><input type="text" name="pp_color" value="'.esc_attr($pp_color).'" size="5"></td>';
	echo '</tr><tr>';
	echo '<td>Inventory:</td><td><select name="pp_inventory" id="pp_inventory">
			<option value="' .__('In Stock', 'pp-plugin'). '" '.(is_null($pp_inventory) || $pp_inventory == __('In Stock', 'pp-plugin') ? 'selected="selected" ' : '').'>' .__('In Stock', 'pp-plugin'). '</option>
			<option value="' .__('Backordered', 'pp-plugin'). '" '.($pp_inventory == __('Backordered', 'pp-plugin') ? 'selected="selected" ' : '').'>' .__('Backordered', 'pp-plugin'). '</option>
			<option value="' .__('Out of Stock', 'pp-plugin'). '" '.($pp_inventory == __('Out of Stock', 'pp-plugin') ? 'selected="selected" ' : '').'>' .__('Out of Stock', 'pp-plugin'). '</option>
			<option value="' .__('Discontinued', 'pp-plugin'). '" '.($pp_inventory == __('Discontinued', 'pp-plugin') ? 'selected="selected" ' : '').'>' .__('Discontinued', 'pp-plugin'). '</option>
		</select></td>';
	echo '</tr>';
	//display the meta box shortcode legend section
	echo '<tr><td colspan="2"><hr></td></tr>';
	echo '<tr><td colspan="2"><strong>' .__('Shortcode Legend', 'pp-plugin') .'</strong></td></tr>';
	echo '<tr><td>' .__('Sku', 'pp-plugin') .':</td><td>[pp show=sku]</td></tr>';
	echo '<tr><td>' .__('Price', 'pp-plugin') .':</td><td>[pp show=price]</td></tr>';
	echo '<tr><td>' .__('Weight', 'pp-plugin') .':</td><td>[pp show=weight]</td></tr>';
	echo '<tr><td>' .__('Color', 'pp-plugin') .':</td><td>[pp show=color]</td></tr>';
	echo '<tr><td>' .__('Inventory', 'pp-plugin') .':</td><td>[pp show=inventory]</td></tr>';
	echo '</table>';
}
...
class pp_widget extends WP_Widget {
	...
	function pp_widget() {
		$widget_ops = array('classname' => 'pp_widget', 'description' => __('Display Post Products','pp-plugin') );
		$this->WP_Widget('pp_widget', __('Post Products Widget','pp-plugin'), $widget_ops);
	}
	...
	function form($instance) {
		...
		?>
			<p><?php _e('Title', 'pp-plugin') ?>: <input class="widefat" name="<?php echo $this->get_field_name('title'); ?>" type="text" value="<?php echo esc_attr($title); ?>" /></p>
			<p><?php _e('Number of Products', 'pp-plugin') ?>: <input name="<?php echo $this->get_field_name('number_products'); ?>" type="text" value="<?php echo esc_attr($number_products); ?>" size="2" maxlength="2" /></p>
		<?php
	}
	...
	function widget($args, $instance) {
		...
		while ($dispProducts->have_posts()) : $dispProducts->the_post(); 
			...
			$pp_price = get_post_meta($post->ID,'pp_price',true);
			$pp_inventory = get_post_meta($post->ID,'pp_inventory',true);
			?><p><a href="<?php the_permalink() ?>" rel="bookmark" title="<?php the_title_attribute(); ?> Product Information"><?php the_title(); ?></a></p><?php
			echo '<p>' .__('Price', 'pp-plugin'). ': ' .$pp_options['currency_sign'].$pp_price .'</p>';
...
function pp_settings_page() {
    ...
    <form method="post" action="options.php">
        ...
        <table class="form-table">
            ...
            <td><input type="text" name="pp_options[currency_sign]" value="<?php echo $pp_currency; ?>" size="1" maxlength="1" /></td>
            </tr>
        </table>

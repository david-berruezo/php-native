<?php
     ...
     *  isTextualData();
     *  
     *  @author http://ukj.pri.ee, http://Dreamplayer.deviantart.com
     */
    class finfo { //extends finfo {       
        ...
        /**
         * Set magic file description array
         * or just use $myfinfo->no_magic['ext']=array();
         * 
         * @param $key unique id in finfo::no_magic array
         * @param $array Like item in finfo::no_magic array
         * @return bool
         */
        public function set_magicDescription($key, $array) {
            if(is_string($key) && is_array($array)){
                if(!array_key_exists($key, $this->no_magic))
                    $this->no_magic[$key] = array(
                        'pattern'      => '',
                        'eval'         => '',
                        'mime_type'    =>'',
                        'fn_extension' => array(),
                ...
                if($mkey=='pattern')unset($this->no_magic[$key]['eval']);
                if($mkey=='eval')unset($this->no_magic[$key]['pattern']);
                return TRUE;               
            }
            return FALSE;
        }
        ...
        public function isTextualData() {
            ...
            if(is_int($slp = strpos($this->mime,'/'))) {
                if( is_int(strpos($this->mime,'xml',$slp)) 
                or is_int(strpos($this->mime,'ecma',$slp))
                or is_int(strpos($this->mime,'rtf',$slp)) 
                or is_int(strpos($this->mime,'php',$slp))

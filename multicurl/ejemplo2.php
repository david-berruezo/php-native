<?php
include('simpledomparser/simple_html_dom.php');
$vectorContent = array();
$mh            = curl_multi_init();
$dom           = new simple_html_dom();
$contador      = 0;

function contentUrls($url){
    global $mh;
    global $vector;
    global $dom;
    global $contador;
    ...
    foreach(array_keys($ch) as $key){
        echo curl_getinfo($ch[$key], CURLINFO_HTTP_CODE)."<br>";
        echo curl_getinfo($ch[$key], CURLINFO_EFFECTIVE_URL)."<br>";
        echo "\n";
        //array_push($vectorContent,curl_multi_getcontent($ch[$key]));
        //$pattern = "/<p ?.*>(.*)<\/p>/";
        //preg_match($pattern, curl_multi_getcontent($ch[$key]), $matches);
        //var_dump($matches);
        $dom         = new DOMDocument();
        $tag         = "p";
        $class       = "amount";
        //preg_match('/<'.$tag.'[^class]+class="'.$class.'"/i',curl_multi_getcontent($ch[$key]),$matches);
        //preg_match('#<p class(.*?)>(.*?)</p>#',curl_multi_getcontent($ch[$key]),$matches);
        preg_match('#<p>(.*?)</p>#',curl_multi_getcontent($ch[$key]),$matches);
        var_dump($matches);

        //$valores = getElementbyclass($class,$tag,curl_multi_getcontent($ch[$key]));
        //var_dump($valores);

        /*
        $dom->loadHTML(curl_multi_getcontent($ch[$key]));
        $xpath       = new DOMXPath($dom);
        $tag         = "p";
        $class       = "amount";
        $consulta    = "//p";
        $elemento    = $dom->getElementsByTagName("p");
        var_dump($elemento);
        if ($elemento->length > 0){
            foreach($elemento as $p){
                if (property_exists($elemento,'nodeName') && $p->getAttribute("class") == "amount" ){
                    echo $p->nodeValue."<br>";
                }
            }
        }
...
function capturarUrl(){
    ...
    $pageSource = htmlspecialchars_decode($pageSource);
    preg_match('#<strong>(.*?)</strong>#',$pageSource,$matches);
    var_dump($matches);
}

function getElementbyclass($elid,$tag="p",$content){
    preg_match_all('/(<'.$tag.'[^>]*>|<\/'.$tag.'>)/si',$content,$openandclosedts);
    $openCloseTags=$openandclosedts[0];
    $found=false;
    $opdivs=1;
    $howmen=-1;
    foreach($openCloseTags as $val){
        if($found) {
            if(trim($val)!="</".$tag.">") {
                $opdivs++;
            } else {
    ...
    preg_match($patterni,$content,$ma);
    $strii="<".$tag.">";

    ...
    do {
        ...
        while($done = curl_multi_info_read($master)) {
            ...
            if ($info['http_code'] == 200)  {
                echo "url: ".$vector[$i]."<br>";
                $output = curl_multi_getcontent($done['handle']);
                $callback($output,$urls[$i]);
                if ( isset($urls[$i+1]) ) {
                    $ch = curl_init();
...
$respuesta    = array();
$respuestaUrl = array();
$resultados   = rolling_curl($vector,"devolucion",array());

function devolucion($output,$url){
    global $respuesta;
    global $respuestaUrl;
    $encontradaUrl = FALSE;
    foreach($respuestaUrl as $resUrl){
        if ($resUrl == $url){
            $encontradaUrl = TRUE;
        }
    }
    if (!$encontradaUrl){
        echo "url2: ".$url."<br>";
        array_push($respuesta,$output);
        array_push($respuestaUrl,$url);
    }
}
?>